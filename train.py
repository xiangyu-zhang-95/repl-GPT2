...
if init_from == 'scratch':
    ...
    print(gptconf)
    if self_impl:
        from model_impl import GPTImpl
        model = GPTImpl(gptconf)
    else:
        from model_orig import GPT
        model = GPT(gptconf)
...
while True:
    ...
    if iter_num % log_interval == 0 and master_process:
        ...
        print(f"time {current_ts}, iter {iter_num}, loss {lossf:.4f}, time {dt*1000:.2f}ms, mfu {running_mfu*100:.2f}%")
    iter_num += 1
    local_iter_num += 1
